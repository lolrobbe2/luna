...
namespace luna
{
	namespace renderer 
	{
		...
		void renderer2D::drawCharQuad(const glm::mat4& transform,const ref<texture> texture,glm::vec4 color, uint64_t handle)
		{
			LN_PROFILE_FUNCTION();
			...
			for (size_t i = 0; i < quadVertexCount; i++)
			{
				...
				vertexPtr->textureIndex = checkHandle(handle);
				vertexPtr->text = true;
				vertexPtr++;
			}
