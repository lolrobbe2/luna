	...
	namespace vulkan
	{
		vulkanTexture::vulkanTexture(const std::string& filePath)
		{
			...
			if(textureFile.is_open() && textureFile.good())
			{
				...
				utils::vulkanAllocator::createBuffer(&buffer,width * height * channels, VkBufferUsageFlagBits::VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VmaAllocationCreateFlagBits::VMA_ALLOCATION_CREATE_WITHIN_BUDGET_BIT);
