...
namespace luna
{
	namespace artemis 
	{
		void descriptorSet::free()
		{
			...
			LN_ERR_FAIL_COND_MSG(m_descriptorSet == VK_NULL_HANDLE, "[Artemis] descriptor set has already be destroyed");
			VkResult freeRes = vkFreeDescriptorSets(*p_device, *p_descriptorPool, 1, &m_descriptorSet);
		...
		descriptorSet::descriptorSet(const VkDevice* p_device,const VkDescriptorSet descriptorSet, const VkDescriptorPool* p_descriptorPool, std::vector<VkWriteDescriptorSet>& descriptorWrites) : p_device(p_device),m_descriptorSet(descriptorSet),p_descriptorPool(p_descriptorPool),descriptorWrites(descriptorWrites)
		{
